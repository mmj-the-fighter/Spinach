...
private:
	...
	//
	spn::Canvas* rtCanvas;
	std::function<rsplitter::Rect()> getTileRenderingTask;
	std::function<void()> notifyTileFinished;
public:
	RayTracer( std::function<rsplitter::Rect()> taskProvider, std::function<void()> taskFinishedNotifier) {
		getTileRenderingTask = taskProvider;
		notifyTileFinished = taskFinishedNotifier;
	}
